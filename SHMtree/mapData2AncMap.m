...
%
%  Tdata = mapData2AncMap(Tdata,NewHeader,AncMap) %Will build refseq by
%  default using current database;
%  Tdata = mapData2AncMap(Tdata,NewHeader,AncMap,'SkipRef') %Will NOT build
%  refseq, since it assumes you already have it.

function Tdata = mapData2AncMap(Tdata,NewHeader,AncMap,varargin)
getHeaderVar;
...
for j = 1:size(Tdata,1)
    if AncMap(j,2) == 0;
        if SkipBuildRef == 0
            Tdata(AncMap(j,1),:) = buildRefSeq(Tdata(AncMap(j,1),:),NewHeader);
        end
    else
        Tdata{AncMap(j,1),RefSeqLoc} = Tdata{AncMap(j,2),SeqLoc};
    end
end
% 
