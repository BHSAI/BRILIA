...
if exist('FileName','var')
    ...
    if H.SeqNumLoc == 0    
        VDJheader = cat(2,VDJheader,{'SeqNum'});
        ...
        if ~isempty(VDJdata{1,H.SeqNumLoc})
            PrevMap = cell2mat(VDJdata(:,H.SeqNumLoc));
            VDJdata(:,H.SeqNumLoc) = num2cell(PrevMap(NTmaps(:,2)));
        else
            VDJdata(:,H.SeqNumLoc) = num2cell(NTmaps(:,2));
        end
    end

    %Add the Group Num information
    GroupNumLoc = findCell(VDJheader,{'GroupNum'});
    if GroupNumLoc == 0
        VDJheader = cat(2,VDJheader,{'GroupNum'});
    ...
    if ispc %Write to Excel if it's a pc
        xlswrite([FilePath OutputFilePre '.xlsx'],cat(1,VDJheader,VDJdata));
    else %Write to csv, tab-delimited file
        writeDlmFile(cat(1,VDJheader,VDJdata),[FilePath OutputFilePre '.csv'],'\t');
    end
end
