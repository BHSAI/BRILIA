...
% VDJdataS = extractClones(VDJdata,VDJheader,Property,Value)
...
if ischar(Input1) %It's probably the file name
    [VDJdata,VDJheader] = openSeqData([Input2 Input1]);
...
PropTable = {'Seq'      H.SeqLoc          0;      
             'RefSeq'   H.RefSeqLoc       0;
             'SeqNum'   H.SeqNumLoc       1;
             'GrpNum'   H.GrpNumLoc       1; 
             'CDR3aa'   H.CDR3Loc(1)      0;
             'CDR3len'  H.CDR3Loc(2)      1;
             'CDR3prop' H.CDR3Loc(1)      0};
...
for k = 1:length(PropNames)
    ...
    if strcmpi(PropTable{PropColLoc(k),1},'CDR3prop')
        TempCDR = VDJdata(:,H.CDR3Loc(1)); %Store it temporarily
        VDJdata(:,H.CDR3Loc(1)) = getPropertyCode(VDJdata(:,H.CDR3Loc(1)));
    end
