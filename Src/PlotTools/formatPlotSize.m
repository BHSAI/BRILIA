...
%fprintf('%s: This will be modified to ONLY set the total figure size,\n and all axes resizing should be done via resizeSubplot.\n,', mfilename);
disp('formatPlotSize will be replaced with resizeFigure and resizeSubplots.');
...
if isempty(Ax)
    ...
    Ax = gca;
elseif ishandle(Ax)
    %Determine if it's a axis or figure handle
    if strcmpi(class(Ax),'matlab.ui.Figure')
        Gx = Ax;
        Ax = get(Gx,'children');
    else
        Gx = get(Ax,'parent');
...
if strcmpi(Option,'inner')
    ...
    Hpos = GxPos(1) - FigW/2;
    if Hpos < 0; Hpos = 0; end
    Vpos = GxPos(2) - FigH/2;
    if Vpos < 0; Vpos = 0; end   
    GxPos = [Hpos Vpos FigW FigH];
    ...
    %If you have zeros for FigW or FigH, set it to current inner size;
    if FigW == 0 || FigH == 0
        CurPos = get(Ax,'position');
    ...
    TightPos = get(Ax,'TightInset') + ones(1,4)*AddBorder; %Thickness of left, bot, right, top border
    ...
    if min(NewAxPos(3:4)) < 0
        disp('formatPlotSize: BorderSize is too large. Final axes size unchanged');
    else
        set(Ax,'Position',NewAxPos);      
    end
end
