...
%
%  savePlot(Gx, Param, Value, ...)
%
%  SaveName = savePlot(Gx, Param, Value, ...)
%
%  P = savePlot('getinput')
%
%  [SaveVarargin, varargin] = savePlot('getvarargin', varargin)
%
%  INPUT
...
%      DPI             Integer      Dots per square inch. Default is 300.
%
%  OUTPUT
...
%
%    Use the 'getvarargin' to parse a varargin and extract param-value
%    fields for savePlot.
%
%  See also getSaveVarargin.
...
if strcmpi(Save, 'y')
    ...
    if isempty(SaveAs) %Need to ask users to select file name
        ...
        [SavePath, SaveFile, SaveExt] = parseFileNameT([SavePath SaveFile], 'ignorefilecheck');
    else
        [SavePath, SaveFile, SaveExt] = parseFileNameT(SaveAs, 'ignorefilecheck');
