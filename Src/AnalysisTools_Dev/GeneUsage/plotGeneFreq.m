%plotGeneFreq will plot a basic bar graph for each VDJ/VK data set on the
%same figure. The number of subplots depends on the data used.

function varargout = plotGeneFreq(S, varargin)
SFields = fieldnames(S);
if ~contains(SFields, 'GeneFreq')
    error('%s: No GeneFreq data provided. Run countGeneFreq before this.', mfilename);
end

error('%s: this code is not completed.', mfilename);
return
